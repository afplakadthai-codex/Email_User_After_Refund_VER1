...
if (!function_exists('bv_order_refund_buyer_detail_url')) {
    function bv_order_refund_buyer_detail_url(int $refundId, int $orderId = 0): string
    {
        $detailPath = bv_order_refund_resolve_existing_page_path([
            '/member/refund_view.php',
            '/member/refunds.php',
        ], '/member/refunds.php');
        $detailUrl = bv_order_refund_build_link($detailPath, $refundId, basename($detailPath) === 'refund_view.php');
        ...
        if ($orderId > 0) {
            $orderDetailPath = bv_order_refund_resolve_existing_page_path([
                '/member/order_detail.php',
                '/member/orders.php',
            ], '/member/orders.php');
            $orderDetailUrl = bv_order_refund_build_link($orderDetailPath, $orderId, basename($orderDetailPath) === 'order_detail.php');
            if (strpos((string)parse_url($orderDetailUrl, PHP_URL_PATH), '/member/') === 0) {
                return $orderDetailUrl;
            }
        }

        $ordersPath = bv_order_refund_resolve_existing_page_path(['/member/orders.php'], '/member/orders.php');
        return bv_order_refund_build_link($ordersPath, 0, false);
    }
}

if (!function_exists('bv_order_refund_buyer_dashboard_url')) {
    function bv_order_refund_buyer_dashboard_url(): string
    {
...
if (!function_exists('bv_order_refund_enforce_role_safe_url')) {
    function bv_order_refund_enforce_role_safe_url(string $url, string $role, int $refundId, int $orderId = 0, bool $dashboard = false): string
    {
        ...
        if ($role === 'buyer') {
            return $dashboard ? bv_order_refund_buyer_dashboard_url() : bv_order_refund_buyer_detail_url($refundId, $orderId);
        }
        if ($role === 'seller') {
...
if (!function_exists('bv_order_refund_resolve_refund_links')) {
    function bv_order_refund_resolve_refund_links(int $refundId): array
    {
        $buyerDetailUrl = bv_order_refund_buyer_detail_url($refundId);
        $buyerDashboardUrl = bv_order_refund_buyer_dashboard_url();
...
if (!function_exists('bv_refund_notification_context')) {
    function bv_refund_notification_context(array $refund, array $order, array $items, array $options = []): array
    {
      ...
      $links = bv_order_refund_resolve_refund_links($refundId);
...
if (!function_exists('bv_refund_notification_build_payloads')) {
    function bv_refund_notification_build_payloads(array $ctx, string $event, array $options = []): array
    {
        ...
        $dashboardUrl = bv_order_refund_enforce_role_safe_url($dashboardUrl, 'buyer', $refundId, $orderId, true);
        $detailUrl = bv_order_refund_enforce_role_safe_url($detailUrl, 'buyer', $refundId, $orderId, false);
